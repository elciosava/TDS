<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        $contato = $_POST['telefone_cliente'] ?? null;
        $cidade = $_POST['cidade_cliente'] ?? null;

        $sql = "INSERT INTO saep_clientes (nome_cliente, email_cliente, telefone_cliente, cidade_cliente) 
                VALUES (:nome, :email, :contato, :cidade)";
    ...
    <div id="container">
        ...
        <div class="formulario">
            ...
            <form action="" method="post">
                ...
                <input type="number" name="telefone_cliente" id="">

                <label for="">Cidade</label>
                <input type="text" name="cidade_cliente" id="">
        ...
        <div class="resultado">
            ...
            <input type="search" name="buscar" id="" placeholder="Digite o nome do cliente">
            <table>
            ...
            <?php
                $sql_busca = "SELECT * FROM saep_clientes";

                $stmt_busca = $conexao->prepare($sql_busca);
                $stmt_busca->execute();


                while($saep_clientes = $stmt_busca->fetch(PDO::FETCH_ASSOC)){
                    echo "<tbody>";
                        echo "<tr>";
                            echo "<td>{$saep_clientes['nome_cliente']}</td>";
                            echo "<td>{$saep_clientes['email_cliente']}</td>";
                            echo "<td>{$saep_clientes['telefone_cliente']}</td>";
                            echo "<td>{$saep_clientes['cidade_cliente']}</td>";

                            echo "<td>
                                    <form action = 'excluir_cadastro.php' method='get'>
                                    <input type='hidden' name='id_cliente' value='{$saep_clientes['id_cliente']}'>
